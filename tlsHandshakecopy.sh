...
MASTER_KEY_FINAL=$(openssl smime -encrypt -aes-256-cbc -in <(echo -n "$MASTER_KEY") -outform DER ~/cert.pem | base64 -w 0)
...
DECODED_ENCRYPTED_SAMPLE_MESSAGE=$(echo $ENCRYPTED_SAMPLE_MESSAGE | base64 -d | openssl enc -d -aes-256-cbc -pbkdf2 -k $MASTER_KEY)
